...
export default function Page() {
  ...
  const d = useMemo(() => {
    ...
    const ordersNeeded =
      inputs.salesGoal / (inputs.aov * (1 + inputs.reorderRate));
    const meetingsNeeded = ordersNeeded / inputs.closeRate;

    const meetingsPerDayPerPerson =
      meetingsNeeded / (weeks * inputs.workdays * inputs.team);
    const emailsPerDayPerPerson =
      (meetingsPerDayPerPerson * inputs.emailShare) /
      inputs.emailToMeeting;
    const callsPerDayPerPerson =
      (meetingsPerDayPerPerson * (1 - inputs.emailShare)) /
      inputs.callToMeeting;

    const meetingsPerWeekTeam =
      meetingsPerDayPerPerson * inputs.workdays * inputs.team;
        ...
        <Card title="Inputs">
          ...
          >
            <Field label="Sales Goal">
              <input
                value={String(inputs.salesGoal)}
                onChange={(e) =>
                  setInputs((s) => ({ ...s, salesGoal: Number(e.target.value || 0) }))
                }
                inputMode="decimal"
                style={{
            ...
            <Field label="Timeline (months)">
              <input
                value={String(inputs.months)}
                onChange={(e) =>
                  setInputs((s) => ({ ...s, months: Number(e.target.value || 0) }))
                }
                inputMode="decimal"
                style={{
                  border: `1px solid ${brand.sand}`,
            ...
            <Field label="Average Order Value">
              <input
                value={String(inputs.aov)}
                onChange={(e) =>
                  setInputs((s) => ({ ...s, aov: Number(e.target.value || 0) }))
                }
                inputMode="decimal"
                style={{
                  border: `1px solid ${brand.sand}`,
            ...
            <Field label="Close Rate (%)">
              <input
                value={String(Math.round(inputs.closeRate * 100))}
                onChange={(e) =>
                  setInputs((s) => ({ ...s, closeRate: Number(e.target.value || 0) / 100 }))
                }
                inputMode="decimal"
                style={{
                  border: `1px solid ${brand.sand}`,
            ...
            <Field label="Email → Meeting (%)">
              <input
                value={String(Math.round(inputs.emailToMeeting * 100))}
                onChange={(e) =>
                  setInputs((s) => ({
                    ...s,
                    emailToMeeting: Number(e.target.value || 0) / 100,
                  }))
                }
                inputMode="decimal"
                style={{
            ...
            <Field label="Call → Meeting (%)">
              <input
                value={String(Math.round(inputs.callToMeeting * 100))}
                onChange={(e) =>
                  setInputs((s) => ({
                    ...s,
                    callToMeeting: Number(e.target.value || 0) / 100,
                  }))
                }
                inputMode="decimal"
                style={{
            ...
            <Field label="Re‑order Rate (%)">
              <input
                value={String(Math.round(inputs.reorderRate * 100))}
                onChange={(e) =>
                  setInputs((s) => ({
                    ...s,
                    reorderRate: Number(e.target.value || 0) / 100,
                  }))
                }
                inputMode="decimal"
                style={{
            ...
            <Field label="Team Size">
              <input
                value={String(inputs.team)}
                onChange={(e) =>
                  setInputs((s) => ({ ...s, team: Number(e.target.value || 0) }))
                }
                inputMode="decimal"
                style={{
                  border: `1px solid ${brand.sand}`,
            ...
            <Field label="Workdays / Week">
              <input
                value={String(inputs.workdays)}
                onChange={(e) =>
                  setInputs((s) => ({ ...s, workdays: Number(e.target.value || 0) }))
                }
                inputMode="decimal"
                style={{
                  border: `1px solid ${brand.sand}`,
            ...
            <Field label="Email Share of Meetings (%)">
              <input
                value={String(Math.round(inputs.emailShare * 100))}
                onChange={(e) =>
                  setInputs((s) => ({
                    ...s,
                    emailShare: Number(e.target.value || 0) / 100,
                  }))
                }
                inputMode="decimal"
                style={{
        ...
        <Card title="Monthly">
          <Rows
            rows={[
              ...
              [
                "Pace (3 mo cumulative)",
                money(d.data.slice(0, 3).reduce((a, b, i) => (i === 0 ? b.cumulative : a + b.cumulative - d.data[i - 1].cumulative), 0)),
              ],
            ]}
          />
        </Card>
  ...
  return (
    <div>
      {rows.map(([k, v]) => (
        ...
        >
          <div style={{ color: brand.slate, fontSize: 13 }}>{k}</div>
          <div style={{ textAlign: "right", fontWeight: 700, color: brand.forest }}>
